...
void PrefillRpcServer::remoteAllocateResource(PrefillGenerateContext& prefill_context) {
    ...
    if (prefill_context.trace_span_guard && prefill_context.trace_span_guard->valid()) {
        ...
        if (client_span != nullptr) {
            ...
            telemetry::injectSpanToClientContext(prefill_context.client_context.get(), client_span);
        }
    }
...
void PrefillRpcServer::pollRemoteOutput(PrefillGenerateContext& prefill_context) {
    ...
    while (prefill_context.client_stream->Read(&response)) {
        ...
        if (!prefill_context.rpc_context.writer->Write(response)) {
            ...
            prefill_context.error_status = responseStreamWriteFailedStatus();
            return;
        }
    }
    CLIENT_GRPC_RET_IF_ERROR(
...
bool PrefillRpcServer::shouldUsePdSeparation(const GenerateConfigPB& generate_config) {
    return generate_config.num_beams() <= 1 && generate_config.variable_num_beams().size() == 0
           && generate_config.num_return_sequences() <= 1 && generate_config.can_use_pd_separation();
}
