...
grpc::Status LocalRpcServer::responseStreamWriteFailedStatus() {
    return grpc::Status(grpc::StatusCode::CANCELLED, "request write outputs pb failed");
}
...
grpc::Status LocalRpcServer::GenerateStreamCall(grpc::ServerContext*                   context,
    ...
    if (mm_processor_ != nullptr && input->multimodal_inputs) {
        RTP_LLM_PROFILE_SCOPE("rpc.mm_update_features");
        auto mm_res = updateMultimodalFeaturesWithTrace(
            input,
            generate_context.trace_span_guard ? generate_context.trace_span_guard->sharedSpan() :
