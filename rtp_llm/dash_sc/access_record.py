...
class GrpcAccessRecord:
    ...
    def capture_response_chunk(
        ...
        *,
        ...
        # Dash-compatible business errors travel inside an otherwise successful
        # gRPC stream.  ``build_dash_error_response`` writes the semantic error
        # number into the inference parameter map and intentionally leaves the
        # legacy top-level ``error_message`` field empty.
        error_no_param = resp.infer_response.parameters.get("error_no")
    ...
    def resolve_status(self, context, exc: Optional[BaseException]) -> float:
        ...
        1. The response carries a protocol/business error, either through the
           top-level ``error_message`` field or a non-zero inner ``error_no``.
           Both may be delivered over a successful gRPC stream.
