...
class OpenTelemetryBootstrapTest {
    ...
    void sharedJsonContract() throws Exception {
        ...
        for (var item : mapper.readTree(Files.readString(fixture))) {
            ...
            if (item.has("error")) {
                var error = assertThrows(TraceConfig.ConfigException.class, () -> TraceConfig.parse(raw), item.toString());
    ...
    @Test
    void regionMappingIsAtomic() throws Exception {
        Path path = directory.resolve("regions.json");
        Files.writeString(path, mapper.writeValueAsString(Map.of("regions", Map.of("cn-test", Map.of(
                "endpoint", "http://localhost/region", "headers", "authorization=Bearer%20fake+x%3D")),
                "fallbacks", Map.of("cn-", "cn-test"))));
        var input = new java.util.HashMap<String, Object>(Map.of("enabled", true, "region", "cn-other",
                "region_config_file", path.toString()));
        TraceConfig config = TraceConfig.parse(mapper.writeValueAsString(input));
        assertEquals("region", config.source());
        assertEquals("Bearer fake+x=", config.headers().get("authorization"));
        assertThrows(UnsupportedOperationException.class, () -> config.headers().put("x", "bad"));
        input.put("endpoint", "http://localhost/manual");
        assertThrows(TraceConfig.ConfigException.class, () -> TraceConfig.parse(mapper.writeValueAsString(input)));
        input.put("headers", Map.of("x", "manual"));
        input.put("region_config_file", "/missing/region/file");
        assertEquals("manual", TraceConfig.parse(mapper.writeValueAsString(input)).source());
    }
    ...
    void resourceIdentityDoesNotReadLegacyOverrides() {
        ...
        var attrs = OpenTelemetryBootstrap.resource(TraceConfig.disabled()).getAttributes();
        ...
        attrs = OpenTelemetryBootstrap.resource(TraceConfig.disabled()).getAttributes();
